...
//创建八叉树
template<class T>
void createOctree(OctreeNode<T> *&root, int maxdepth, double xmin, double xmax, double ymin, double ymax, double zmin,
                  double zmax) {
    //cout<<"处理中，请稍候……"<<endl;
...
void find(OctreeNode<T> *&p, double x, double y, double z) {
    ...
    if (x <= p->xmin + txm && x >= p->xmax - txm && y <= p->ymin + tym && y >= p->ymax - tym && z <= p->zmin + tzm &&
        z >= p->zmax - tzm) {
